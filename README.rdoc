...
If you want to make a query that searches across multiple parameters you can add
parameters with the `#add_param` method. Once all desured parameters are set,
use `#go!` to trigger it and get the parsed response.
    Crunchbase::API.add_param("geo", "Boston")
    Crunchbase::API.add_param("range", 50)
    boston_startups = Crunchbase::API.go!
    Crunchbase::API.clear_params
